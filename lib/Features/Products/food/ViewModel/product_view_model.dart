...
class ProductViewModel extends ChangeNotifier {
  ...
  Future<List<FoodItemModel>> getProductCategories(
    ...
    if (res != null) {
      _foodProducts = res
          ...
          .map<FoodItemModel>(
              (data) => FoodItemModel.fromMap(data)) // Convert to model
          .toList();
      print(_foodProducts);
      notifyListeners();
    }
  ...
  getProductDetail(productId) async {
    await _productRepository.fetchfoodDetail(productId).then((v) {
      foodDetail = v;
      notifyListeners();
    });
  }
